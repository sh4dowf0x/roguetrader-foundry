/**
 * Rogue Trader | Import Mutations To Compendium
 *
 * Usage:
 ...
 *
 * Re-running the macro updates existing mutation items by name/type.
 */

const TARGET_PACK_ID = "roguetrader.character-creation-options";
const DEFAULT_MUTATION_ICON = "icons/svg/blood.svg";
...
async function runImport() {
  ...
  const table = game.roguetrader.references.getTable("mutations");
  if (!table?.entries?.length) {
    ui.notifications?.error("Rogue Trader | No mutation reference entries were found.");
    return;
  }
  ...
  for (const entry of table.entries) {
    const itemData = game.roguetrader.references.buildItemData("mutations", entry.min, {
      type: "mutation",
      name: entry.name,
    ...
    if (!itemData) {
      console.warn("Rogue Trader | Failed to build mutation item data for entry:", entry);
      continue;
    }

    itemData.img = itemData.img || DEFAULT_MUTATION_ICON;
    ...
    const existing = index.find((doc) => doc.type === "mutation" && doc.name === itemData.name);
    if (existing) {
  ...
  const message = `Rogue Trader | Mutation import complete. Created ${created}, updated ${updated}.`;
